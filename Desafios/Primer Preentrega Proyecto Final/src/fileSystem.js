...
class ProductManager {
    ...
    addProducts = async (title, description, price, thumbnail, stock, id) => {
        try {
            ...
            fs.appendFile('./src/products.json', JSON.stringify(product, null, 2))
        } catch (err) {
            console.log(err);
        }    
    }
    ...
    createFile = async () => {
        try {
            ...
            const data = await fs.writeFile('./src/products.json', `${products}`,'utf8')
            return data
        } catch (err) {
            console.log(err)
        }
    ...
    deleteAll = async () => {
        try {
            fs.unlink('./src/products.json', err => {
                if(err) {
                    console.log(err)
                } else {
